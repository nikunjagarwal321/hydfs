...
func (ds *DistributedSystemService) MultiAppend(req *MultiAppendRequest, resp *MultiAppendResponse) error {
	ConsolePrintf("Received MultiAppend request: HyDFSfilename=%s, LocalFilename=%s", req.HyDFSFileName, req.LocalFileName)
	...
	ConsolePrintf("MultiAppend completed: %s", resp.Message)
	return nil
}
...
func (s *Server) StartRPCServer() error {
	...
	go s.deleteFilesOutsideMyRange()
